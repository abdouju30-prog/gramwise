...
function normalizeSession(raw: WizardSession): WizardSession {
  return {
    ...raw,
    fixedCharges: normalizeFixedChargesForm(raw.fixedCharges),
    ...(raw.recipe ? { recipe: normalizeRecipeForm(raw.recipe) } : {}),
  };
}
...
export function loadWizardSession(): WizardSession | null {
  ...
  try {
    ...
    if (rawV2) return normalizeSession(JSON.parse(rawV2) as WizardSession);

    const rawV1 = sessionStorage.getItem(STORAGE_KEY_V1);
    if (!rawV1) return null;

    const legacy = normalizeSession(JSON.parse(rawV1) as WizardSession);
