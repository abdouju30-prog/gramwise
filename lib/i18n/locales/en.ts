import type { LocaleBundle } from "../types";

export const en: LocaleBundle = {
  meta: {
    title: "GramWise — Pastry costing calculator",
    description:
      "Spread monthly overhead, cost your recipes, and price with margin on selling price.",
  },
  brand: { tag: "Pastry costing" },
  lang: { choose: "Language" },
  currency: {
    choose: "Entry currency",
    legend: "Currency",
    hint: "All amounts (overhead, ingredients, wages) are entered in this currency.",
  },
  nav: {
    progress: "Progress",
    stepFixed: "Fixed charges",
    stepRecipe: "Recipe",
    stepResults: "Results",
    openCalculator: "Open calculator",
    home: "Home",
    account: "Account",
  },
  beta: {
    bannerStrong: "Free beta",
    bannerText: "— try the calculator and compare to your usual numbers.",
    bannerLink: "Tester guide →",
    feedbackTitle: "Beta — do the numbers match?",
    feedbackBody:
      "Compare this result to your spreadsheet or notebook. A 2-minute reply helps us validate before paid launch.",
    whatsappSend: "Send feedback (WhatsApp)",
    guideLink: "Beta guide",
    form: "Form",
    pageEyebrow: "Morocco beta",
    pageTitle: "Pastry testers (5 spots)",
    pageLead:
      "Goal: confirm costs and recommended price match your reality (Excel, notebook, invoices). Calculator is free — no payment during beta.",
    serverOff: "Server beta mode off — this guide still works for manual tests.",
    step1Title: "1. Open the tool",
    step1Cta: "Start — fixed charges",
    step2Title: "2. Flow (15–25 min)",
    step2Items: [
      "Monthly fixed charges + capacity (batches or hours)",
      "One real recipe: ingredients, time, margin, waste",
      "Results: compare to what you charge today",
    ],
    step3Title: "3. Send feedback",
    step3Body:
      "Success = “numbers match” on materials, labor, fixed charges, and price order of magnitude.",
    whatsappCta: "Feedback via WhatsApp",
    whatsappMissing: "Founder: set NEXT_PUBLIC_BETA_WHATSAPP on Vercel.",
    footHome: "Home",
    whatsappMessage: (appUrl) =>
      [
        "Hi — GramWise pastry beta test.",
        `Link: ${appUrl}/start`,
        "After my test, numbers match? ☐ Yes ☐ No",
        "Gap (1 line): ",
        "Recipe tested: ",
      ].join("\n"),
  },
  landing: {
    eyebrow: "For artisan pastry businesses",
    heroBefore: "Know your real cost before you",
    heroAccent: "set the price",
    lead: "GramWise spreads overhead across production, builds recipe cost line by line, and recommends a selling price — no spreadsheet guesswork.",
    openCalculator: "Open the calculator",
    betaTesters: "Beta testers",
    whyTitle: "Why bakers use it",
    benefits: [
      {
        title: "Numbers you can defend",
        body: "Engine validated against 10 Excel reference recipes — materials, labor, and fixed load spelled out.",
      },
      {
        title: "Margin on selling price",
        body: "Target margin % on the price tag, not markup-on-cost.",
      },
      {
        title: "Built for pastry",
        body: "Spread monthly rent and utilities across batches or shop hours, then price each recipe run.",
      },
    ],
    howTitle: "How it works",
    steps: [
      "Enter monthly fixed charges and capacity",
      "Add ingredients, labor phases, waste %",
      "Get break-even and recommended selling price",
    ],
    footer: "GramWise · Pastry costing · Engine tested on reference cases ·",
    footerApp: "App",
    pricingBetaTitle: "Beta — free",
    pricingBetaNote:
      "Calculator is open with no payment. We validate numbers with 5 bakers before any paid offer.",
    guideTester: "Tester guide (15 min)",
    pricingTitle: "Pricing",
    pricingNoteReady: "Secure checkout via Stripe.",
    pricingNoteGumroad:
      "Checkout on Gumroad (card, PayPal). Gumroad is the seller of record — ideal before you form a company. The calculator stays free to use.",
    pricingNoteFree: "Calculator is free until checkout is configured.",
    lifetimeLabel: "Best for solo shops",
    lifetimeTitle: "Lifetime",
    lifetimePrice: "29",
    lifetimeOnce: "once",
    lifetimeFeatures: ["Unlimited recipes", "Full cost breakdown", "No subscription"],
    lifetimeCta: "Get lifetime access",
    monthlyLabel: "Flexible",
    monthlyTitle: "Monthly",
    monthlyPeriod: "/ month",
    monthlyFeatures: ["Same features", "Cancel anytime"],
    monthlyCta: "Subscribe monthly",
    checkoutSoon: "Checkout soon",
    freeCalculator: "Use the free calculator now",
  },
  start: {
    eyebrow: "Calculator",
    title: "Your costing workflow",
    lead: "Three steps — fixed charges, recipe, then results with a full breakdown.",
    step1Title: "Fixed charges",
    step1Desc: "Monthly overhead + ingredient prices",
    step2Title: "Recipe",
    step2Desc: "Ingredients, batches or prep runs, margin",
    step3Title: "Results",
    step3Desc: "Breakdown and recommended price",
    step4Title: "Monthly report",
    step4Desc: "All recipes + fixed charges",
    cta: "Begin step 1",
  },
  monthlyReport: {
    eyebrow: "Overview",
    title: "Monthly report",
    lead: "See what you actually spend each month.",
    hint: "Fixed charges plus every saved recipe at its monthly volume. Fixed costs are counted once and split by production volume.",
    summaryTitle: "Monthly total",
    fixedCharges: "Fixed charges (rent, utilities…)",
    sumMaterials: "Materials (all recipes)",
    sumLabor: "Labor (all recipes)",
    sumFixedShare: "Share of fixed charges (allocated)",
    grandTotal: "Estimated monthly spend",
    incompleteWarning:
      "Some recipes are incomplete (ingredients, active hours, or monthly volume) and are excluded from the total.",
    tableTitle: "Per recipe",
    tableHint: "Volume = batches / prep runs per month or equivalent (hours mode).",
    colRecipe: "Recipe",
    colVolume: "Volume / month",
    colMaterials: "Materials / month",
    colLabor: "Labor / month",
    colFixedShare: "Fixed share",
    colTotal: "Total / month",
    volumeBatches: "batches",
    volumePreparations: "prep",
    volumeRuns: "runs",
    statusIncomplete: "Incomplete",
    emptyRecipes:
      "Save at least one recipe (Save on the recipe step) to fill the report.",
    goRecipes: "My recipes",
    backStart: "Calculator home",
    exportCsv: "Export CSV",
  },
  fixed: {
    eyebrow: "Step 1 of 3",
    title: "Fixed charges",
    lead: "List your monthly bills — rent, utilities, insurance, and more.",
    explainShort: "Monthly overhead and ingredient purchase prices.",
    monthlyLegend: "Monthly overhead",
    monthlyTotalLabel: "Total / month",
    monthlyHint: "Fill each line — the total updates automatically.",
    chargeCategory: "Category",
    chargeAmount: "Amount",
    chargeRent: "Rent",
    chargeEnergy: "Utilities",
    chargeInsurance: "Insurance",
    chargeSubscriptions: "Subscriptions",
    chargeMarketing: "Marketing",
    chargeCustomPlaceholder: "Other expense",
    addChargeLine: "Add another line",
    removeChargeLine: "Remove this line",
    monthlyPreviewCaption:
      "Spread per recipe in the next step (batches or prep runs).",
    monthlyPreviewError: "Enter at least one valid amount to continue.",
    smigLegend: "Minimum wage — labor cost",
    smigHint:
      "Required: enter your country’s legal minimum (SMIG, SMIC, etc.). Examples are not used in calculations.",
    smigMonthlyLabel: "Gross minimum / month",
    smigMonthlyPlaceholder: "e.g. 3119 (Morocco)",
    smigHoursLabel: "Reference hours / month",
    smigHoursPlaceholder: "e.g. 191",
    smigHourlyPreview: "Estimated hourly cost: {rate}",
    smigHourlyInvalid: "Enter the minimum and hours (> 0) to continue.",
    catalog: {
      legend: "Ingredient list (purchase prices)",
      hint: "Set prices once — they auto-fill in every recipe.",
      summaryCount: "{n} ingredients",
      summaryEmpty: "Open to add prices",
      searchLabel: "Search",
      searchPlaceholder: "e.g. flour, butter…",
      expandHint: "Click here to show and edit the list",
      openListAria: "Open ingredient list",
      searchEmpty: "No matching ingredient.",
      name: "Ingredient",
      namePlaceholder: "e.g. Flour",
      unit: "Unit",
      price: "Price / unit",
      add: "Add ingredient",
      remove: "Remove",
    },
    back: "Back",
    continue: "Continue to recipe",
  },
  recipe: {
    eyebrow: "Step 2 of 3",
    title: "Recipe",
    lead: "Ingredients, labor, waste and margin for this product.",
    loadPreset: "Load cupcakes preset",
    nameLabel: "Recipe name",
    namePlaceholder: "e.g. Cupcakes",
    titleHeader: {
      chooseRecipe: "Choose a recipe",
      newRecipe: "New recipe",
      presetCupcakes: "Example — Cupcakes",
      iconLabel: "Name this recipe",
      deleteRecipe: "Delete",
      unnamedRecipe: "Untitled",
      loadedHint: "Saved recipe loaded — see results below, or pick New recipe to edit.",
    },
    capacity: {
      title: "Spread fixed costs on this recipe",
      hint: "How much of this product do you make per month?",
      modeGroup: "Count by",
      unitBatches: "Batches",
      unitPreparations: "Prep runs",
      unitHours: "Hours",
      countLabelBatches: "Batches / month (this recipe)",
      countLabelPreparations: "Prep runs / month (this recipe)",
      shopHours: "Shop hours / month",
      perBatch: "per batch",
      perPreparation: "per prep run",
    },
    ingredientsLegend: "Ingredients",
    ingredientsHint:
      "Quantity in g, ml, or each — purchase price stays per kg, L, or each (auto-converted).",
    ingredientName: "Name",
    ingredientNamePlaceholder: "e.g. Flour",
    defaultIngredientNames: [
      "Flour",
      "Sugar",
      "Butter",
      "Eggs",
      "Milk",
      "Baking powder",
      "Cream",
      "Chocolate",
      "Salt",
      "Vanilla",
    ],
    defaultLaborPhaseLabels: ["Prep", "Baking", "Decorating", "Packaging"],
    qty: "Qty",
    qtyUnit: "Unit",
    qtyUnitHint: "Quantity and cost use the same unit.",
    costPerUnit: "Purchase price",
    costPerUnitHint: "Price per kg, L, or each — qty in g or ml is converted (e.g. 800 g × 11/kg = 8.80).",
    lineCost: "Line cost",
    import: {
      lead: "Photo, file or pasted text — we organize your ingredients.",
      photo: "Photo",
      file: "File",
      pasteLabel: "Text",
      pastePlaceholder: "Paste your list here (e.g. 500 g flour, 2 L milk…)",
      organize: "Organize",
      scanning: "Reading…",
      parseEmpty: "Nothing found. Add name + quantity + unit (kg, L, each).",
      fileError: "Could not read file.",
      photoError: "Could not read photo — paste the text.",
      previewTitle: (count) =>
        `${count} ingredient${count === 1 ? "" : "s"}`,
      apply: "Add to list",
      units: {
        kg: "kg",
        g: "g",
        L: "L",
        ml: "ml",
        unit: "each",
      },
    },
    removeIngredient: "Remove ingredient",
    addIngredient: "+ Ingredient",
    laborLegend: "Hands-on labor (SMIG)",
    laborHint:
      "Count only time when the worker is tied up and cannot do something else in parallel.",
    laborHintActive:
      "Active hours: mixing, watching the oven, decorating… Not passive duration (e.g. oven 2 h but only 30 min of real work).",
    laborPassiveExample:
      "e.g. bake 2 h in the oven but 0.5 h of actual work → enter 0.5 h, not 2 h.",
    laborSmigRateNote: "Hourly rate from minimum wage (step 1): {rate}. Leave blank to use it.",
    laborSmigMissing:
      "Enter your country’s minimum wage on Fixed charges first — required for labor costing.",
    laborByOwner: "Work done by the owner",
    laborByOwnerHint:
      "Hourly cost cannot be below SMIG (even if no salary is paid).",
    phase: "Step",
    activeHours: "Active hours",
    hours: "Active hours",
    ratePerHour: "MAD / hour",
    rateSmigDefaultHint: "Default: SMIG",
    phasePlaceholder: "e.g. Decorating",
    hoursPlaceholder: "e.g. 2",
    ratePlaceholder: "e.g. 120",
    removePhase: "Remove step",
    addLabor: "+ Add a step",
    pricingLegend: "Waste and margin",
    wasteLabel: "Waste (%)",
    wasteHint: "Lost or thrown-away ingredients. e.g. 3 = 3% extra on materials.",
    marginLabel: "Target margin (%)",
    marginHint:
      "Your share of the selling price. e.g. 40% means the price covers costs plus 40% of the price for you.",
    previewTitle: "Batch preview",
    materials: "Materials",
    labor: "Labor",
    fixedLoad: "Fixed load",
    fullCost: "Full cost",
    previewError: "Fill ingredients, labor, waste and margin to preview.",
    back: "Back",
    viewResults: "View results",
    saveAndNew: "Save · new recipe",
    saveChanges: "Save changes",
    savedToast: "Recipe saved.",
    updatedToast: "Recipe updated.",
    library: {
      expand: (count) => `My recipes (${count})`,
      delete: "Delete",
      exportFile: "Export to my PC",
      privateNote:
        "Recipes stay on this device, private to your browser profile. Multiple users won't erase each other. Export a backup to be safe.",
    },
  },
  results: {
    eyebrow: "Step 3 of 3",
    title: "Results",
    lead: "Full cost breakdown and recommended selling price.",
    defaultRecipeName: "Your recipe",
    recommendedPrice: "Recommended selling price",
    breakEvenSub: "Break-even",
    breakdownTitle: "Cost breakdown",
    directMaterials: "Direct materials",
    ingredientNamesTitle: "Ingredients entered",
    ingredientUnnamed: "Ingredient",
    directLabor: "Direct labor",
    fixedLoad: "Fixed load allocated",
    fullCostBreakEven: "Full cost (break-even)",
    marginWaste: "Margin {margin}% on selling price · waste {waste}%",
    marginTipTitle: "How margin works",
    marginTipBody:
      "Recommended price = full cost ÷ (1 − margin%). This is not markup-on-cost at the same percentage.",
    exportTitle: "Export",
    exportCsv: "Export CSV",
    exportPdf: "Export PDF",
    calcError: "Could not calculate — check your inputs on previous steps.",
    editRecipe: "Edit recipe",
    editFixed: "Edit fixed charges",
    viewMonthlyReport: "Monthly report",
    fxNote:
      "Amounts entered in dirhams (MAD). Euro and dollar use indicative rates below.",
  },
  checkout: {
    eyebrow: "Payment",
    successTitle: "Thank you",
    successBody:
      "Gumroad emails your receipt with a license key. Paste it below to unlock the calculator on this device.",
    cancelTitle: "Checkout cancelled",
    cancelBody: "No charge was made. You can buy later from the home page.",
    openCalculator: "Open calculator",
    unlockLater: "Activate later",
    home: "Home",
    viewPricing: "View pricing",
  },
  unlock: {
    eyebrow: "License",
    title: "Activate GramWise",
    lead: "Paste the license key from your Gumroad purchase email.",
    formTitle: "License key",
    formHint: "One key per purchase — activate again on other devices when needed.",
    licenseLabel: "License key",
    licensePlaceholder: "Paste Gumroad license key",
    submit: "Activate",
    submitting: "Verifying…",
    errorInvalid: "Invalid or expired key. Check your Gumroad email or contact support.",
    buyLink: "Buy a license",
    home: "Home",
  },
};
